...
class AgentJdbcTemplateRepositoryTest {
    ...
    void shouldFindHazel() {
        //hazel.getAgencies.get(0)

        Agent hazel = repository.findById(1);
        assertEquals(1, hazel.getAgentId());
        assertEquals("Hazel", hazel.getFirstName());
        assertEquals(2, hazel.getAgencies().size()); //grabbing related table information to make sure it mapped correctly
                                                               //hazel should be associated with 2 agencies
                                                             //related table is accessed in Agent class via getAgencies
    }
